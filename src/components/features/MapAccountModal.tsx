...
// Import Polkadot account + CHAINS
...
export default function MapAccountModal({ onClose }: MapAccountModalProps) {
  ...
  // Convert CHAINS → array usable by dropdown
  const AVAILABLE_NETWORKS = Object.values(CHAINS).map((chain) => ({
    id: chain.id,
    name: chain.name,
    symbol: chain.nativeCurrency.symbol,
    icon: chain.chainIconUrl,
  }))
        ...
        >
          ...
          >
            <div className="flex items-center gap-4">
              <Image
                src={selectedNetwork ? selectedNetwork.icon : "/wallets/polkadot.svg"}
                alt={selectedNetwork ? selectedNetwork.name : "Polkadot"}
                width={36}
                height={36}
          ...
          <AnimatePresence>
            {isDropdownOpen && (
              <motion.div
                className="bg-[#1A1D23] border border-white/10 rounded-xl p-2 flex flex-col gap-1"
                ...
                {AVAILABLE_NETWORKS.map((net) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-gray-400 text-left text-xs">{net.symbol}</span>
                    </div>
                  </button>
                ))}
              </motion.div>
          ...
          <button
            ...
            className={`
              w-full py-2 rounded-xl font-medium transition-all duration-200 disabled:opacity-50
